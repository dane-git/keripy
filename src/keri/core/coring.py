...
class Saider(Matter):
    ...
    def _saidify(cls, sad, *, code=MtrDex.Blake3_256, kind=None, label='d', ignore=None, path=[],**kwargs):
        ...
        final_sad = sads[-1]
        final_saider = saiders[-1]
        for sad in sads:
            if [sad.label] == sad.path:
                if final_sad != sad:
                    print(3982, 'ERROR wrong: final_sad is wrong!')


        # saiders.append()?
        return final_saider, final_sad.ked, sads
    ...
    @classmethod
    def _compactify(cls, sad: dict, label: str = Saids.d):
        """
        Recursively compacts nested dictionaries or lists to a specified label if it exists.

        Parameters:
            sad (dict): The dictionary to compact.
            label (str): The field name to compact on (e.g., 'd').
